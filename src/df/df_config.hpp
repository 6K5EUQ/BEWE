...
struct Config {
    ...
    // MRC: Kraken 5채널을 하나로 합쳐 IQ 링에 넣는다 (df_mrc.hpp). 기본 꺼짐 —
    // 링 앞에서 합치므로 가중치 하나가 캡처 대역 전체를 지배하고, 그 성격이
    // 주파수에 따라 갈린다: 300 MHz 아래에선 대역 내 전 신호가 +7 dB 를 받지만
    // 그 위에선 가장 센 방위로 향한 빔이 되어 다른 방위 약신호를 잃는다.
    bool     mrc_enable = false;
