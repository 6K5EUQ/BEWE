...
// 창 = [관측최솟값 - 5 dB, 0 dB]. 하한만 신호에 따라 움직이고 상한은 고정이다.
//
...
//
// 상한은 신호가 없으면 -30 dB 고정이다. 예전엔 0 dB 였는데 RF 는 거기까지 오지
// 않아 화면 위쪽 30 dB 가 늘 비어 있었다. 고정값이라 눈금이 대역·시각과 무관하게
// 같아서 화면끼리 바로 비교된다.
// 피크가 그 위로 올라오면 10 dB 눈금으로 올림해 천장을 올린다 — 안 그러면 초과분이
// 양자화에서 잘리고, 캡처 쪽 천장감시가 10 초마다 재조정을 걸어 HIST 파일이 계속
// 갈린다. 눈금을 거칠게 잡는 이유도 그 파일 분할을 막기 위해서다.
//
...
inline void autoscale_db_window(float noise, float peak, float lo,
    ...
    // 신호가 없을 때의 고정 천장. 예전엔 0 dB 였는데 RF 는 거기까지 오지 않아
    // 화면 위쪽 30 dB 가 늘 비어 있었고, 그만큼 노이즈가 아래로 눌렸다.
    out_max = -30.f;

    // 그 위로 실제 신호가 있으면 천장을 올린다. 안 올리면 초과분이 양자화
    // 단계에서 255 로 잘리고(net_server.cpp broadcast_fft), 캡처 쪽 감시가
    // 10 초마다 재조정을 걸며, 그 재조정이 양자화 범위를 바꿔 HIST 파일까지
    // 가른다 (long_waterfall.cpp 의 quant-rotate).
    // 10 dB 눈금으로 올림해서 여유를 둔다 — 피크가 조금 흔들릴 때마다 천장이
    // 따라 움직이면 같은 파일 분할이 계속 일어난다.
    if(peak > out_max) out_max = std::ceil(peak / 10.f) * 10.f;

    // 하한이 상한을 넘어서면 창이 뒤집힌다 (색 계산이 0 나눗셈으로 깨진다).
    // 정상 RF 에서는 안 생기지만, 파일 재생·합성 데이터가 양수 dB 를 줄 수 있다.
    if(out_min > out_max - 20.f) out_min = out_max - 20.f;
}
