...
//
// 상한 0 dB 고정: RF 신호는 실제로 0 dBFS 근처까지 오지 않으므로 화면 위쪽은
// 대체로 비어 있고, 그만큼 노이즈가 아래로 눌린다. 대비를 잃는 대신 눈금이
// 대역·시각과 무관하게 늘 같아서 화면끼리 바로 비교되고, 강한 신호가 갑자기
// 들어와도 창을 다시 잡을 일이 없다 (재조정마다 HIST 파일이 갈리는 것도 없다).
//
// 세 경로가 반드시 같은 창을 써야 한다 — 어긋나면 같은 신호가 HOST·JOIN·HIST 에서
// 다른 색으로 보인다.
inline void autoscale_db_window(float noise, float peak, float lo,
                                float& out_min, float& out_max){
    (void)peak;                                  // 상한이 고정이라 쓰지 않는다
    // 관측 최솟값보다 조금 아래에서 시작 (가장자리가 창 밖으로 나가지 않게)
    if(!(lo < noise)) lo = noise - 1.f;          // 방어: lo 가 이상하면 noise 기준
    out_min = lo - 5.f;
    out_max = 0.f;

    // 하한이 0 을 넘어서면 창이 뒤집힌다 (색 계산이 0 나눗셈으로 깨진다).
