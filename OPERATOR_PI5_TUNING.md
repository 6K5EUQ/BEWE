...
*(현재 코드 기준선: v13.2.0 — 2026-07-15. 코드 최적화를 먼저 배포한 뒤 적용한다 —
그래야 측정 기준선이 오염되지 않는다.)*

BEWE 코드 변경 없이 OS 쪽에서 얻을 수 있는 절감.

대상: DGS-1/2/3 (Pi5 헤드리스 HOST, `build-cli` = `BEWE_HEADLESS`, pad=1).
Central(raspb2)은 상시 부하가 작아 이 runbook 대상은 아니다 — 단, v13.2.0 부터
Central 은 HIST 파일 finalize(정시 롤오버·CF/SR 변경) 때마다 **백그라운드 스레드에서
zstd 압축**을 돌린다. 이때만 수 초간 한 코어에 스파이크가 뜬다(수신 핫패스는 별
스레드라 안 막힘). 상시 부하가 아니므로 governor/전력 대상은 여전히 아니다.
