...
def decide(logits: np.ndarray, temperature: float, taus, classes):
    """logits [C] -> (status, pred_mmsi, conf_decipct). conf scaled *1000, i.e.
    percent with 1 decimal place (0..1000) — must fit the wire u16 field."""
    z = logits / max(temperature, 1e-6)
    z -= z.max()
    p = np.exp(z); p /= p.sum()
    c = int(p.argmax())
    conf = int(round(1000.0 * float(p[c])))
    if float(p[c]) >= taus[c]:
