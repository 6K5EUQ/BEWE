...
def _demo_items(cfg):
    it = []
    # 좌표는 korea_osm_data(OSM 해안선) even-odd 수역 검증 통과분 — 전 꼭짓점·선분·
    # 투영선·구역이 육지 250m 이상 이격 (scratchpad water_new.py 로 재검증 가능).
    # 1) 충돌위험: 진해만 수역 스트립(35.11N) 동서 상호 접근 — 완만한 호+타각 요동
    #    (실항적 느낌: 침로가 서서히 휘고 점간 미세 지터). 연장 교점 = (35.1107, 128.6430)
    a = [(35.1075, 128.6100), (35.1082, 128.6140), (35.1086, 128.6180), (35.1092, 128.6222),
         (35.1095, 128.6262), (35.1100, 128.6300), (35.1102, 128.6335)]
    b = [(35.1130, 128.6740), (35.1126, 128.6700), (35.1124, 128.6655), (35.1119, 128.6610),
         (35.1117, 128.6565), (35.1113, 128.6520), (35.1110, 128.6480)]
    it.append(("path", 999000001, "HAEJIN NO.7", a))
    it.append(("path", 999000002, "SEONGWOO 3", b))
    it.append(("alert", 1, 3, 999000001, 999000002, 35.1107, 128.6430, 92.0, 140.0, 210.0,
               "CPA 140m 3.5분 후: HAEJIN NO.7 x SEONGWOO 3", "양 선박 VHF16 호출, 우현 변침 지시"))
    # 2) 좌초위험: REEF-A(35.055~070N 128.640~662E) 동측에서 서진, 진입점 = 동쪽 경계
    c = [(35.0650, 128.7030 - 0.0066 * i) for i in range(6)]
    it.append(("path", 999000003, "DAEYANG 21", c))
    it.append(("alert", 2, 2, 999000003, 0, 35.0650, 128.6622, 78.0, -1.0, 300.0,
               "REEF-A 5분 내 진입: DAEYANG 21", "즉시 변침 지시, 암초 회피"))
    # 3) 이상항적: 정상 NE 직진 → 표류 → 선회 루프 (조타불능/배회 실패턴, 남측 개활수역)
    d = [(35.0200, 128.5850), (35.0225, 128.5890), (35.0250, 128.5930), (35.0275, 128.5970),
         (35.0295, 128.6015), (35.0310, 128.6070),
         (35.0332, 128.6106), (35.0361, 128.6095), (35.0385, 128.6119), (35.0382, 128.6158),
         (35.0355, 128.6176), (35.0328, 128.6158), (35.0325, 128.6119), (35.0349, 128.6095),
         (35.0378, 128.6106)]
    it.append(("path", 999000004, "GEUMRYONG HO", d))
    it.append(("alert", 3, 2, 999000004, 0, d[-1][0], d[-1][1], 88.0, -1.0, -1.0,
               "이상항적 88점: GEUMRYONG HO", "항적 감시, VHF 호출"))
    # 4) 위장의심: 가덕수로 남측(34.98~34.99N 128.80~83E) 북서진
    e = [(34.9800 + 0.0024 * i, 128.8300 - 0.0060 * i) for i in range(6)]
    it.append(("path", 999000005, "MYEONGSUNG 5", e))
    it.append(("alert", 4, 3, 999000005, 0, e[-1][0], e[-1][1], 95.0, -1.0, -1.0,
               "RF지문 불일치 6/6: MYEONGSUNG 5", "MMSI 위장 의심 — VHF·레이더 확인"))
    return it
